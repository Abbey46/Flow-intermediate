// Import the FungibleToken library from version 0x06
import FungibleToken from 0x05

//SunCoin contract: Implements a fungible token on the Flow blockchain
pub contract  SunCoin: FungibleToken {

    // Total supply of SunCoin
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, VaultInterface {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @SunCoin.Vault
        ...
        // Destroy the vault
        destroy() {
           SunCoin.totalSupply =SunCoin.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
           SunCoin.totalSupply =SunCoin.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&SunCoin.Minter>(/public/Minter, target: /storage/MinterStorage)
